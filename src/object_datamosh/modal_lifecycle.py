...
class ModalOperationLifecycle:
    ...
    def begin(
        ...
        *,
        ...
        runtime.run_identity = self._run_identity_factory()
    ...
    def finalize(self, phase: OperationPhase, status: str) -> None:
        ...
        try:
            ...
            if self._timer is not None and self._window_manager is not None:
                with suppress(AttributeError, ReferenceError, RuntimeError):
            ...
            if self._progress_started and self._window_manager is not None:
                ...
                self._progress_started = False
            terminal_phase = OperationPhase.FAILED if cleanup_error is not None else phase
            terminal_status = (
                f"Cleanup failed: {cleanup_error}" if cleanup_error is not None else status
            )
            try:
